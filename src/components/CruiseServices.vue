  ...
  <tiny-modal @close="$emit('input', false)">
    ...
    <template #body>
      <ul class="ces-cruise__list">
        ...
        <li v-if="value.includedServices.length">
          <div class="ces-bold">Включено в тариф:</div>
          <ul class="ces-cruise__list">
            <li v-for="service in value.includedServices" :key="`${service.code}`">
        ...
        <li>
          ...
          <ul class="ces-cruise__list">
            <li v-for="cancel in value.cancellationFees" :key="cancel.datefrom">
              {{
        ...
        <li v-text="value.nameChangeTerms"></li>
      </ul>
    </template>
  </tiny-modal>
</template>
